...
def create_user(name="UnName", age=0, sex="o", *others):
...
def create_data_set():
    ...
    while (True):  # while for create new user in data_set
        break_point = 0
        others = {}
        while (int(break_point) != -1):  # while for others details
            data = input("If you wont other data. Enter name data (to Exit -1): ")
            if data == "-1":
                break_point = data
                if int(break_point) == -1:
                    break
...
def main():
    ...
    dict_male_female = split_male_female(data_set)
    for k, v in dict_male_female.items():
        print(k)
        for x, y in v.items():
            print(x, y)
    ...
    dict_median_avg = find_median_average(data_set)
